...
<?php 
  ...
  $sql="SELECT * FROM tpublicity,login WHERE login.user_name = '{$_SESSION['user_name']}' AND login.uindex = tpublicity.uindex AND tpublicity.is_deleted = 0";

  if ($result=mysqli_query($connection,$sql))
    {
    // Return the number of rows in result set
    $rowcount=mysqli_num_rows($result);
    $sql2="SELECT * FROM tpublicity,login WHERE tpublicity.done='Finish' AND login.user_name = '{$_SESSION['user_name']}' AND login.uindex = tpublicity.uindex AND tpublicity.is_deleted = 0 ";
  ...
  $query = "SELECT * FROM tpublicity,login WHERE login.user_name = '{$_SESSION['user_name']}' AND login.uindex = tpublicity.uindex AND tpublicity.is_deleted = 0   ORDER BY tpublicity.id";
 ...
 /* $user = mysqli_fetch_assoc($tasks);
  ...
  if (isset($_POST['fsubmit'])) {
    ...
    $query = "UPDATE tpublicity SET done = 'Finish' ,situation = 'Finish' WHERE id = $task_list1 LIMIT 1";
