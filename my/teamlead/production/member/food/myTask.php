...
<?php 
  ...
  $sql="SELECT * FROM tfood,login WHERE login.user_name = '{$_SESSION['user_name']}' AND login.uindex = tfood.uindex AND tfood.is_deleted = 0";

  if ($result=mysqli_query($connection,$sql))
    {
    // Return the number of rows in result set
    $rowcount=mysqli_num_rows($result);
    $sql2="SELECT * FROM tfood,login WHERE tfood.done='Finish' AND login.user_name = '{$_SESSION['user_name']}' AND login.uindex = tfood.uindex AND tfood.is_deleted = 0 ";
